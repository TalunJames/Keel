...
// Keel: mount under /periscope when embedded in the portal.
const PS_BASE = (typeof window !== 'undefined' && window.__PERISCOPE_BASE__) || '';
function psPath(p) { return PS_BASE + p; }
function psPathname() {
  const p = (typeof location !== 'undefined' && location.pathname) || '/';
  if (PS_BASE && p.startsWith(PS_BASE)) return p.slice(PS_BASE.length) || '/';
  return p;
}

const SAMPLE_PDF_URL = psPath('/uploads/Listening_FSS_CSSD_Tier3_1_1c.pdf');
...
function makeLibraryClient(getToken) {
  ...
  // Keel: session cookie auth — uploads use the portal login, not a bearer token.
  const apiFetch = (url, opts = {}) => fetch(psPath(url), { credentials: 'include', ...opts });
  return {
    async authStatus() {
    ...
    async remove(filename) {
      const res = await apiFetch(`/api/uploads/${encodeURIComponent(filename)}`, {
        method: 'DELETE',
        headers: authHeaders(),
      });
      if (!res.ok) {
...
const MailerApp = () => {
  ...
  const handleShareLink = useCallback(async () => {
    try {
      ...
      const link = location.origin + (url.startsWith('/') ? url : psPath('/' + url));
...
const App = () => {
  ...
  const enterMailer = useCallback(async (accessCode) => {
    ...
    setAccessBusy(true);
    try {
      const res = await fetch(psPath(`/api/shares/${encodeURIComponent(code)}`), { credentials: 'include' });
      if (!res.ok) throw new Error();
      history.pushState(null, '', psPath(`/s/${code}`));
      setShowMailer(true);
    } catch {
      setAccessError(true);
    } finally {
